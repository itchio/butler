...
// Generated at 2018-02-23 13:13:00.3944401 +0100 CET m=+0.011530401
package messages
...
func EnsureAllRequests(router *buse.Router) {
  ...
  if _, ok := router.Handlers["PickUpload"]; !ok { panic("missing request handler for (PickUpload)") }
  if _, ok := router.Handlers["GetReceipt"]; !ok { panic("missing request handler for (GetReceipt)") }
  ...
  if _, ok := router.Handlers["PickManifestAction"]; !ok { panic("missing request handler for (PickManifestAction)") }
  if _, ok := router.Handlers["ShellLaunch"]; !ok { panic("missing request handler for (ShellLaunch)") }
  if _, ok := router.Handlers["HTMLLaunch"]; !ok { panic("missing request handler for (HTMLLaunch)") }
  if _, ok := router.Handlers["URLLaunch"]; !ok { panic("missing request handler for (URLLaunch)") }
  if _, ok := router.Handlers["SaveVerdict"]; !ok { panic("missing request handler for (SaveVerdict)") }
  if _, ok := router.Handlers["AllowSandboxSetup"]; !ok { panic("missing request handler for (AllowSandboxSetup)") }
  if _, ok := router.Handlers["PrereqsFailed"]; !ok { panic("missing request handler for (PrereqsFailed)") }
  ...
  if _, ok := router.Handlers["Test.Double"]; !ok { panic("missing request handler for (Test.Double)") }
}
