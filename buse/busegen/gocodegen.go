...
func (bc *BuseContext) GenerateGoCode() error {
	bc.Task("Generating go code")

	doc := bc.NewDoc("../messages/messages.go")

	doc.Line("// Code generated by busegen; DO NOT EDIT.")
	doc.Line("// Generated at %s", time.Now())
	...
	doc.Line("	%#v", "errors")
	doc.Line("")
	doc.Line("	%#v", "github.com/itchio/butler/buse")
	...
	for _, category := range scope.categoryList {
		...
		for _, entry := range cat.entries {
			...
			case EntryKindParams:
				...
				case CallerClient:
					doc.Line("")
					doc.Line("func (r *%s) Register(router *buse.Router, f func(*buse.RequestContext, *%s) (*%s, error)) {", typeName, paramsTypeName, resultTypeName)
					doc.Line("  router.Register(%#v, func (rc *buse.RequestContext) (interface{}, error) {", method)
					doc.Line("    var params %s", paramsTypeName)
					doc.Line("    err := json.Unmarshal(*rc.Params, &params)")
					doc.Line("    if err != nil {")
					doc.Line("    	return nil, &buse.RpcError{Code: jsonrpc2.CodeParseError, Message: err.Error()}")
					doc.Line("    }")
				...
				case CallerServer:
					doc.Line("")
					doc.Line("func (r *%s) Call(rc *buse.RequestContext, params *%s) (*%s, error) {", typeName, paramsTypeName, resultTypeName)
			...
			case EntryKindNotification:
				...
				doc.Line("}")
				doc.Line("")
				doc.Line("func (r *%s) Notify(rc *buse.RequestContext, params *%s) (error) {", typeName, paramsTypeName)
	...
	doc.Line("")
	doc.Line("func EnsureAllRequests(router *buse.Router) {")
